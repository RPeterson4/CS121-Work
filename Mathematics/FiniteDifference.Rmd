...
f <- function(x){ exp(-x/5)*sin(x) }

##Implementing the symbolic derivative
df <- function(x){exp(-x/5) * cos(x) - exp(-x/5) * (1/5) * sin(x)}

##Plot that shows value of Df() and finite-difference value found by applying evalD() to f
g <- function(x){ evalD(f,x) }
plotFun( f(x)~x, x.lim=c(0,10),col='red')
plotFun( g(x)~x, x.lim=c(0,10),col='blue',add=TRUE)

##Plot that displays arithmetic difference between symbolic derivative and finite-difference derivative
...
g2 <- function(x){ evalD2(f,x,1e-20) }
g3 <- function(x){ evalD2(f,x,1e-15) }
g4 <- function(x){ evalD2(f,x,1e-10) }
g5 <- function(x){ evalD2(f,x,1e-5) }
g6 <- function(x){ evalD2(f,x,.1) }
g7 <- function(x){ evalD2(f,x,1) }
...
## Task III
```{r}
g10 <- function(x){evalD2(f,x,h=1e-8*abs(x))}
g10(0)
```
